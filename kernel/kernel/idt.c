...
void irqDivError();
void irqStepOver();
void irqNMIInterrupt();
void irqBreakPoint();
void irqOverflow();
void irqBoundTest();
void irqInvalidOperation();
void irqNoCoprocessor();
void irqDoubleFault();
void irqCoprocessorOutofBound();
void irqInvalidTSS();
void irqNoSegment();
void irqStackSegmentFault();
void irqPageFault();
void irqTime();
...
void initIdt() {
	...
	setTrap(idt + 0x4, SEG_KCODE, (uint32_t)irqOverflow, DPL_KERN);
	setTrap(idt + 0x5, SEG_KCODE, (uint32_t)irqBoundTest, DPL_KERN);
	setTrap(idt + 0x6, SEG_KCODE, (uint32_t)irqInvalidOperation, DPL_KERN);
	setTrap(idt + 0x7, SEG_KCODE, (uint32_t)irqNoCoprocessor, DPL_KERN);
	setTrap(idt + 0x8, SEG_KCODE, (uint32_t)irqDoubleFault, DPL_KERN);
	setTrap(idt + 0x9, SEG_KCODE, (uint32_t)irqCoprocessorOutofBound, DPL_KERN);
	...
	setIntr(idt + 0x20, SEG_KCODE, (uint32_t)irqTime, DPL_USER);
